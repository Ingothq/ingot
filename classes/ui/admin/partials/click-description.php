<?php
/**
 * Partial returned via API for click testing descriptions
 *
 * @package   ingot
 * @author    Josh Pollock <Josh@JoshPress.net>
 * @license   GPL-2.0+
 * @link
 * @copyright 2016 Josh Pollock
 */
?>
<h3>
	<?php _e( 'Content tests change some type of content -- text, colors, images, etc. to see what leads to the most conversions.', 'ingot' ); ?>
</h3>
<h4>
 <?php _e( 'Currently Supported Content Test Types', 'ingot' ); ?> :</h4>
<ul>
	<li>
		<em><?php _e( 'Link', 'ingot' ); ?>:</em> <?php _e( 'Test the text of a link.', 'ingot' ); ?>
	</li>
	<li>
		<em><?php _e( 'Button', 'ingot' ); ?>:</em> <?php _e( 'Test the text of a button.', 'ingot' ); ?>
	</li>
	<li>
		<em><?php _e( 'Button Color', 'ingot' ); ?>:</em> <?php _e( 'Test the color of a button.', 'ingot' ); ?>
	</li>
</ul>
<h4><?php _e( 'Content Test Types Coming Soon', 'ingot' ); ?>:</h4>
<ul>
	<li>
		<em><?php _e('Images & Featured Images', 'ingot' ); ?></em>
	</li>
	<li>
		<em><?php _e( 'Forms & Form Layouts', 'ingot' ); ?></em>
	</li>
	<li>
		<em><?php _e( 'Sales Funnels', 'ingot' ); ?></em>
	</li>
</ul>
