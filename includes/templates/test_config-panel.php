...
<div class="ingot-config-group ingot-click-test-only">
	...
	<select style="width:395px;" placeholder="<?php _e( 'Select Type', 'ingot' ); ?>" id="ingot-test_config-click_type" name="test_config[click_type]" required="required">
		...
		foreach($types as $id => $option ){
			...
			printf( '<div id="%1s" class="ingot-test_config-click_type-desc" style="visibility:%2s;" aria-hidden="%3s" >%4s</div>', esc_attr( 'ingot-click-type-desc-' . $id ), $visibility, $aria, $option[ 'desc' ] );
		}
		?>
	...
	<div class="ingot-config-group">
		...
		<input id="ingot-test_config-inital_tests" type="number" class="regular-number" name="test_config[inital_tests]" value="{{test_config/inital_tests}}" required="required" min="1">
		<p class="description" style="margin-left: 190px;">
			<?php _e( 'How many tests to run before weighting one option over another?', 'ingot' ); ?>
		</p>
	</div>
	...
	<div class="ingot-config-group">
		...
		<input id="ingot-test_config-threshold" type="number" class="regular-number" name="test_config[threshold]" value="{{test_config/threshold}}" required="required" min="0">
		<p class="description" style="margin-left: 190px;">
			<?php _e( 'What percentage of tests must a option win to be considered the winner?', 'ingot' ); ?>
		</p>
	</div>
	...
	<div class="ingot-config-group ingot-click-test-only" id="ingot-test_config-click_target-wrap">
		...
		<p class="description" style="margin-left: 190px;">
			<?php _e( 'What selector should we use to track clicks?', 'ingot' ); ?>
		</p>
	</div>
